...
with lib;
{
  ...
  mkSystemdService = {
    ...
    # `RestartSec = 5` only ~2 starts land inside a 10s window, so the default
    # limit is UNREACHABLE: a unit whose failure is PERMANENT restarts forever,
    # in `activating`, which `systemctl --failed` does not list. Nothing
    # surfaces it.
    #
    # MEASURED 2026-08-05 on rio, on a unit generated by THIS helper:
    # `wadachi-daemon` reached 531 restarts and climbing (~1374 journal lines
    # per ten minutes) because one unreadable directory under its watch root
    # made the failure permanent. A systemd USER unit, so invisible to the
    # system manager's failed list as well. Three siblings the same night
    # (`sui-cache-serve`, `pleme-gitops`, `seed-grafana-*` at NRestarts=10854
    # over ~15h) were the same miscalibration in hand-written units.
    #
    # 3 starts / 300s against RestartSec=5: three attempts span ~10-15s, well
    # inside the window, so a permanent fault reaches `failed` in under a
    # minute. A transient fault (one crash then a clean start) never approaches
    # it, so ordinary resilience is unchanged — this bounds only the
    # pathological case.
    #
    # OVERRIDABLE on purpose: a unit whose permanent death is worse than its
    # looping legitimately wants unbounded retry. On rio the fleet guard found
    # exactly that shape in upstream nixpkgs units — `sshd` and `k3s` ship
    # `StartLimitIntervalSec=0` deliberately, because a permanently-dead sshd
    # locks you out of the box. Note `0` DISABLES rate limiting rather than
    # tightening it.
    #
    # These are [Unit] keys. In [Service] systemd logs "Unknown key name" and
    # IGNORES them — the limit would read as set and enforce nothing, which is
    # the same invisible-failure class this closes.
