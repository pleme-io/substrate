...
in runTests [
  ...
  # failing unit restarts forever in `activating` — a state `systemctl
  # --failed` does not list. `wadachi-daemon`, generated by THIS helper,
  # reached 531 restarts on rio before anyone saw it.
  #
  # The property under test is not "the numbers are 300 and 3" — it is that
  # `burst` attempts fit INSIDE the window with room to spare. Asserting the
  # relation rather than the literals means retuning restartSec cannot silently
  # re-open the hole.
  (let svc = serviceHelpers.mkSystemdService {
    name = "limited"; description = "d"; command = "/bin/t";
  };
  u = svc.systemd.user.services.limited;
  in mkTest "systemd-start-limit-is-reachable"
    (u.Unit ? StartLimitIntervalSec
      && u.Unit ? StartLimitBurst
      && u.Unit.StartLimitIntervalSec > u.Service.RestartSec * u.Unit.StartLimitBurst
      && u.Unit.StartLimitIntervalSec != 0
      && u.Unit.StartLimitBurst != 0)
    "burst attempts must fit inside the window, so a permanent failure reaches `failed`")

  # The keys belong to [Unit]. Rendered into [Service], systemd logs "Unknown
  # key name" and IGNORES them — the limit would read as set and enforce
  # nothing, which is the same invisible-failure class. Pinned so a future
  # refactor cannot quietly relocate them.
  (let svc = serviceHelpers.mkSystemdService {
    name = "placed"; description = "d"; command = "/bin/t";
  };
  u = svc.systemd.user.services.placed;
  in mkTest "systemd-start-limit-lives-in-the-unit-section"
    (!(u.Service ? StartLimitIntervalSec) && !(u.Service ? StartLimitBurst))
    "StartLimit* in [Service] is silently ignored by systemd")

  # Overridable, because a unit whose permanent death is worse than its looping
  # (sshd, k3s) legitimately wants unbounded retry — and on rio the guard found
  # exactly those upstream units at IntervalSec=0 deliberately.
  (let svc = serviceHelpers.mkSystemdService {
    name = "override"; description = "d"; command = "/bin/t";
    startLimitIntervalSec = 600; startLimitBurst = 9;
  };
  u = svc.systemd.user.services.override;
  in mkTest "systemd-start-limit-is-overridable"
    ((u.Unit.StartLimitIntervalSec or null) == 600 && (u.Unit.StartLimitBurst or null) == 9)
    "a caller must be able to widen or disable the bound deliberately")
