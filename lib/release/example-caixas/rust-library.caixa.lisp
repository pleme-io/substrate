...
(defcaixa my-rust-lib
  ...
  :ci-config {
    :bump {:default-type "patch" :skip-when-no-source-changes true}
    :publish {:no-verify true}
    :security {:fail-on-severity "high" :check-license-headers true :license "MIT"}}
  ...
  :docs {:enabled true :format :cargo-doc :branch "gh-pages"}

  ;; Optional: post release events to Slack
  :notify {:slack {:webhook-secret "SLACK_WEBHOOK_URL"}}) ;; Renderer: ;;   cd my-rust-lib/ ;;   pleme-release render-caixa rust-library.caixa.lisp ;; ;; Output (committed to repo root): ;;   Cargo.toml ;;   .pleme-io-release.toml ;;   .github/workflows/auto-release.yml ;;   .github/workflows/pre-merge-gate.yml ;;   .github/workflows/security-gate.yml ;; ;; Operator never edits yaml. The caixa is the source of truth.
