...
(defcaixa my-npm-pkg
  ...
  :ci-config {:bump {:default-type "patch"} :publish {:access "public"}}

  :workflows [:auto-release :pre-merge-gate :security-gate]

  ;; Optional: render TypeDoc + deploy to gh-pages
  :docs {:enabled true :format :typedoc}) ;; Renders to: ;;   package.json ;;   .pleme-io-release.toml ;;   .github/workflows/*.yml  (consumer shims)
