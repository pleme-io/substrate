...
(defaction npm-bump
  ...
  :inputs {
    :bump-type {:default "patch" :description "patch | minor | major"}
    :skip-when-no-source-changes {:default "true"}
    :source-paths {:default "src package.json package-lock.json"}}
  :outputs {
    :bumped {:type :bool :description "true if a bump happened"}
  ...
  :body (let
    ((bump-type (config-resolve "BUMP_TYPE" "bump" "default-type" "patch"))
      (skip-flag
        (config-resolve
    ...
    (cond
      ((not (npm-should-bump? skip-flag source-paths)) (emit-skip old-version))
      (else
        (let* ((bump-r (npm-version-bump bump-type)) (new-version (npm-package-version)))
          (cond
            ((not (= (status-of bump-r) 0)) (emit-failure "npm version failed" old-version))
            ((equal? new-version old-version)
              (emit-failure (string-append "version unchanged: " old-version) old-version))
            (else (emit-success old-version new-version))))))))
