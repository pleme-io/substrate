...
(defaction cargo-bump
  ...
  :inputs {
    ...
    :skip-when-no-source-changes {
      :default "true"
      :description "Skip when no source changed since last tag"}
  ...
  :body (let
    ...
    (cond
      ((not (cargo-should-bump? skip-flag source-paths)) (emit-skip old-version))
      (else
        (let* ((bump-r (cargo-set-version-bump bump-type))
           (new-version (cargo-package-version)))
          (cond
            ((not (= (status-of bump-r) 0)) (emit-failure "cargo set-version failed" old-version))
