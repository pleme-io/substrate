...
(defaction slack-notify
  ...
  :inputs {
    :webhook-url {:required true :description "Slack incoming-webhook URL (from secrets)"}
    :title {:required true}
    :body {:default ""}
  ...
  :body (let
    ((webhook (env-required "WEBHOOK_URL"))
      (title (env-required "TITLE"))
      (body (env-get "BODY" ""))
      (color (env-get "COLOR" "good"))
      (fields (env-get "FIELDS" "[]")))
    (let*
      ((payload (build-slack-payload title body color fields))
        (code (http-post-status webhook payload)))
      (cond
        ((or (equal? code "200") (equal? code "204")) (append-output "delivered=true") (exit 0))
        (else
