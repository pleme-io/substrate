...
in {
  ...
  mkCrate2nixDockerImage = {
    ...
    # ★ Phase E2 — OCI Image Spec v1.1 reserved annotations. Sourced
    # from the shared `mkStandardLabels` helper so EVERY substrate
    # builder (Rust + Node + Ruby + Wasm) emits the same shape.
    standardLabels = (import ../../util/docker-helpers.nix).mkStandardLabels {
