# FIXTURE — the TRUE legacy lib/blackmatter-component-flake.nix, frozen.
#
# On 2026-06-12 the live lib/blackmatter-component-flake.nix was retired and
# became a thin delegation shim over iroha.mkComponentFlake (the blackmatter
# swallow). This file is the byte-preserved original implementation (only the
# ./util/test-helpers.nix relative import is re-pointed for the new location)
# so the iroha parity suite (tests/component-flake.nix) keeps comparing v2
# against the TRUE legacy semantics forever — including the captured legacy
# eval-nixos-module breakage (stub prefix conflict) and the silent drop of
# unknown args / unknown modules.* keys. Do not "fix" this file; its bugs are
# load-bearing test oracles.
#
...
let
  ...
  # Fixture relocation: was `./util/test-helpers.nix` when this file lived at
  # lib/blackmatter-component-flake.nix; re-pointed to the same file from
  # lib/iroha/tests/fixtures/. Semantics unchanged.
