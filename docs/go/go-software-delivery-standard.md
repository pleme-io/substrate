...
## Dimension: Platform Tool Generation (TOOLGEN)

This dimension is the **never-produce-a-§4-tool-by-hand-again guarantee** — the
capstone over the whole standard. Every other dimension says *how a Go tool must
be shaped*; TOOLGEN says *a §4 tool is no longer shaped by a human at all*. It is
**authored as a typed spec and forged**, then the only thing a human writes is the
small, typed seam that holds the tool-specific (and backend-binding) logic.

The **platform engine** is `go-tool-synthesizer`
([repo](https://github.com/pleme-io/go-tool-synthesizer)), owned by
`PRIM-go-tool-synthesizer`. It mechanizes the §4 canonical shape (§3.5
signatures) and the two seams; the `pleme-doc-gen` `render_go` forge compiles the
embedded `(defgotool …)` spec from a `(defcaixa :ecosystem :go :tool-spec …)`
`caixa.lisp`, lowers it through the engine, pins the toolchain, and runs `gofmt`.

**TOOLGEN-01 — author + forge, never hand-write.** A §4 Go tool MUST be authored
as a typed `(defgotool …)` `GoToolSpec` and FORGED. The spec is the single typed
input: `name` / `kind` (one of `Cli` / `Service` / `Daemon` / `Action` / `Binary`
/ `Library`) / `binary_name` / `profile` / `primitives` / `config_fields`
(`+:default`) / `commands` / `api_ops`. The forge lowers it to the exact §4 shell
— `main.go` (the `errs.Exit(run(ctx))` funnel + `borealis.Execute` outermost),
`internal/app/config.go` (the typed shikumi `Config` + `shikumi.For[Config]…Load`),
`internal/app/app.go` (the cli-go App tree), `internal/app/errors.go` (the
errors-go vocabulary). Hand-writing any of these is forbidden; every generated
file carries `// Code generated by go-tool-synthesizer (defgotool: <name>). DO NOT
EDIT.` so the drift checker can prove provenance.

**TOOLGEN-02 — re-forge-clean shell; all logic behind a seam.** NEVER hand-edit a
generated file. The shell is re-forge-clean: `pleme-doc-gen caixa --source
caixa.lisp --out . --force` regenerates it byte-for-byte. ALL tool-specific logic
lives behind a typed SEAM:

  - the **`clientFactory` seam** in the engine-generated `internal/app/client.go`
    — the abstract `Client` interface (one method per `api_op`) plus `NewClient`,
    which delegates to a package-level `clientFactory`. Emitted whenever the spec
    declares `api_ops`.
  - the **`reconcilerFactory` seam** in the engine-generated
    `internal/app/reconciler.go` — `buildReconciler(cfg, log)` delegating to a
    package-level `reconcilerFactory`, returning a no-op default until registered.
    Emitted for a controller `Service` (a `Service` kind composing
    `controller-go`).

The concrete logic — and the **only** backend binding in the whole repo — lives in
a hand-written `*_adapter.go` (`client_adapter.go`, and `reconciler_adapter.go`
for controllers) that registers via `func init() { clientFactory = … }` /
`func init() { reconcilerFactory = … }`. Because `init` runs at import time, the
registration survives re-forging the seam file — exactly like the M3 absorption
layer. The `*_adapter.go` is the ONLY non-generated and the ONLY backend-binding
file; it carries no `DO NOT EDIT` header.

**TOOLGEN-03 — worlds-separate is engine-enforced at render.**
`go-tool-synthesizer::lower_guarded` scans every generated SHELL file
(case-insensitively) for the active profile's `forbidden_substrings(profile)` and
FAILS the forge with a `WorldsSeparateViolation` (naming the file + the offending
token + a leak hint) on the first leak. A profile declares its own forbidden
substrings — a private/closed-world profile forbids its backend's name; the
public/default world forbids nothing — and a new forbidden world is added by
extending the profile match, never by per-emitter string checks. `caixa-validate`
runs the forge in the same guarded mode, so a backend name that slipped into spec
text (a command `:long` / `:summary` / `:usage`, or the tool `:description`) is
caught BEFORE the shell is written, not only by a downstream test. The generated
shell names NO backend; only the hand-written `*_adapter.go` may.

**TOOLGEN-04 — standardized by construction.** The forge output is standardized,
not standardized-after-the-fact: the `go` directive is pinned to the fleet
standard (`go-tool-synthesizer::DEFAULT_GO_VERSION`) and a matching `toolchain
go<VER>` line is emitted in BOTH `go.mod` and `go.work`, so `go mod tidy` / `go
work sync` can never silently bump the tool to a dependency's higher directive
(after which `GOTOOLCHAIN=local` refuses to build); the output is gofmt-canonical
(`pleme-doc-gen` runs `gofmt -w` as a single typed spawn post-pass — never a
reimplementation of the formatter); the released-binary name is the spec's
`binary_name` slot (`resolved_binary_name`), so the published binary may differ
from the Go module leaf; and `DefaultConfig()` is SEEDED only from each config
field's declared `:default` — a field without `:default` takes the Go zero value,
NEVER the field name as a placeholder (the historical placeholder bug).

**TOOLGEN-05 — private tool = public shell + private adapter.** A PRIVATE tool is
a PUBLIC/generic forged shell + a PRIVATE hand-written `*_adapter.go`; the
visibility split is structural, not textual. The shell stays
open-source-publishable because it names no backend (TOOLGEN-03); only the adapter
that registers the seam is private. The repo's visibility is declared in
`repo-forge` `repos.lisp` via `(defrepo … :visibility :private)` and reconciled by
the GitHub-posture IaC — NEVER `gh repo create` / `gh repo edit` / the web UI.
`caixa-validate` cross-checks the on-disk repo against its `repos.lisp`
`:visibility` record; a private Go-tool repo MUST have exactly one `repos.lisp`
record and at least one hand-written `*_adapter.go` while the shell stays
re-forge-clean (TOOLGEN-02).

> **The contract names no backend.** Everything above is generic. A backend-bound
> solution-set (a private tool whose adapter speaks one vendor SDK) is only an
> *example* of TOOLGEN-05's private-adapter case — it appears in the
> `borealis-pattern-registry.md` build-queue table (the private analysis), never
> in a normative TOOLGEN rule. The engine, the seams, and the guard are
> vendor-blind by construction.
