...
- **Pillar 9** (SDLC): `rust-tool-release-flake.nix`, `rust-workspace-release-flake.nix`, `rust-service-flake.nix`, `rust-library.nix`, `ruby-gem-flake.nix`, `wasi-service-flake.nix`, `wasi-service-flux-flake.nix`, `tatara/program-flake.nix`, `build/swift/ios-game-flake.nix` (the iOS-game SDLC devloop — cross-compile Rust → iOS, host test/lint, deploy to the simulator "VM" or a tethered phone via the Xcode impurity boundary; emits the guided `nix run .#{sdlc,test,lint,build-sim,run-sim,build-device,game-device,game}` app distribution; reference consumer `pleme-io/asobi`) — every repo's `flake.nix` anchors on one of these. `nix run .#app` / `nix run .#test` / `nix run .#release` uniformity comes from here.
